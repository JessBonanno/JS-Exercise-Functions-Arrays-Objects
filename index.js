...
function temperatureInF(/* code here */ temp, unit) {
  ...
  if (unit === "F") {
    ...
    const conversion = (temp * 9) / 5 + 32;
    return `${Math.round(conversion)}F`;
  }
}
...
function getCarInfoById(/* code here */ inventory, id) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].id === findId) {
      var make = inventory[i].car_make;
      var model = inventory[i].car_model;
    }
...
function carMaker(/* code here */ num) {
  ...
  const movingCar = {
    ...
    drive: function(distance) {
      movingCar.odometer = movingCar.odometer + distance;
      return movingCar.odometer;
    }
  };
  return movingCar;
}
